...
/* Includes ------------------------------------------------------------------*/
#include "DLCh/usbd_cdc_if.h"

/* USER CODE BEGIN INCLUDE */
#include "main.h"   // 包含 main.h 以调用 Update_Signal_Parameters 等函数
#include <stdio.h>  // 用于 sprintf, sscanf
#include <string.h> // 用于 strncmp, strlen
#include <stdlib.h> // 用于 atof (如果使用)
...
#ifndef USB_RX_BUFFER_SIZE_IF
#define USB_RX_BUFFER_SIZE_IF APP_RX_DATA_SIZE // 使用头文件中定义的大小
#endif
// 用于发送确认/错误消息的临时缓冲区
char cdc_if_tx_buffer[64];
/* USER CODE END PV */
...
static void Process_CDC_Data(uint8_t *Buf, uint32_t Len);
